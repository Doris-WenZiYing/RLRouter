...
# ====== 真 BFS 串接：把 RL 的任務丟給 BFS 實跑 ======
def _make_bfs_tuples_from_rl_sf(sf_objs):
    """
    把 RL 的 SF 物件/tuple 轉成 BFS 期望的 tuple:
    (sf_id:str, compute:float, power:float, memory:int, threads:int)
    支援：
      - RL 的類別物件（有屬性 sf_id / id / name、compute_power/compute、power/power_watts、memory/mem、threads）
      - 你 BFS 本來用的 tuple：('SF1', 1e8, 30, 40, 1)
    """
    sf_list_tuples, sf_id2tuple = [], {}

    for sf in sf_objs:
        if isinstance(sf, tuple) or isinstance(sf, list):
            # BFS 樣式
            if len(sf) < 5:
                raise ValueError(f"SF tuple 長度不足 5: {sf}")
            sf_id   = str(sf[0])
            compute = float(sf[1])
            power   = float(sf[2])
            memory  = int(sf[3])
            ...
            # RL 物件樣式
            sf_id = (
                getattr(sf, 'sf_id', None)
                or getattr(sf, 'id', None)
                or getattr(sf, 'name', None)
            )
            compute = (
                getattr(sf, 'compute_power', None)
                or getattr(sf, 'compute', None)
            )
            power = (
                getattr(sf, 'power', None)
                or getattr(sf, 'power_watts', None)
            )
            memory = (
                getattr(sf, 'memory', None)
                or getattr(sf, 'mem', None)
            )
            threads = getattr(sf, 'threads', 1)

            missing = [k for k, v in [('sf_id', sf_id), ('compute', compute),
                                      ('power', power), ('memory', memory)] if v is None]
            if missing:
                raise ValueError(f"SF 轉換失敗: 欄位 {missing} 為 None (sf={sf})")
        ...
        tup = (sf_id, compute, power, memory, threads)
        sf_list_tuples.append(tup)
        sf_id2tuple[sf_id] = tup

    return sf_list_tuples, sf_id2tuple


def _make_bfs_topology_from_rl(topology, sf_id2tuple):
    """
    RL 的 topology -> BFS 的 topology（ConnectedSF 必須是 BFS 形式的 tuple）
    """
    topo = {}
    for r, info in topology.items():
        topo[r] = {
            'Neighbors': list(info.get('Neighbors', [])),
            'ConnectedHosts': list(info.get('ConnectedHosts', [])),
            'ConnectedSF': []
        }
        for sf in info.get('ConnectedSF', []):
            if isinstance(sf, (tuple, list)) and len(sf) >= 5:
                # 已經是 BFS tuple
                tup = (str(sf[0]), float(sf[1]), float(sf[2]), int(sf[3]), int(sf[4]))
            else:
                # RL 物件：用 id/name 去對應前一步做好的字典
                sf_id = (
                    getattr(sf, 'sf_id', None)
                    or getattr(sf, 'id', None)
                    or getattr(sf, 'name', None)
                )
                tup = sf_id2tuple.get(str(sf_id))
            if tup is not None:
                topo[r]['ConnectedSF'].append(tup)
            # 若 None 就跳過（或你想 raise 也可）
    return topo
...
def get_task_io_size(t):
    """支援 RL Task 物件或 (task_id, host, Di, Fi, Mi[, Tmax]) tuple。"""
    if hasattr(t, "data_size"):
        return t.data_size
    if isinstance(t, (tuple, list)) and len(t) >= 3:
        return t[2]
    return None

def run_real_bfs_on_tasks(tasks):
    """用真 BFS 在同一批 RL 任務上實跑，回傳 {'success_rate','avg_delay','avg_power'}。"""
    import copy, io, sys
    import bfs_7sf_5rd as bfs  # ←←← 這裡名稱要和你的 BFS 檔名一致（不含 .py）

    # 用 RL 的設定建立 BFS 需要的輸入（確保兩邊一致）
    topology, roadm_distances, wavelength_capacity, sf_objs = create_sample_configurations()
    sf_list_tuples, sf_id2tuple = _make_bfs_tuples_from_rl_sf(sf_objs)
    topology_bfs = _make_bfs_topology_from_rl(topology, sf_id2tuple)

    # RL Task 物件 → BFS 期待的五元組 + Tmax_list
    Tmax_list = [getattr(t, 'max_delay', t[5] if isinstance(t, (tuple, list)) and len(t) >= 6 else None)
                 for t in tasks]
    Task_list = [(getattr(t, 'task_id', t[0]),
                  getattr(t, 'source_host', t[1]),
                  get_task_io_size(t),
                  getattr(t, 'required_compute', t[3] if isinstance(t, (tuple, list)) and len(t) >= 4 else None),
                  getattr(t, 'memory_requirement', t[4] if isinstance(t, (tuple, list)) and len(t) >= 5 else None))
                 for t in tasks]

    # 防呆：若有 None 會讓 BFS 爆掉，直接丟出錯
    for i, (tid, host, Di, Fi, Mi) in enumerate(Task_list):
        if None in (tid, host, Di, Fi, Mi) or Tmax_list[i] is None:
            raise ValueError(f"Task 轉換失敗，第{i+1}筆有 None：{(tid, host, Di, Fi, Mi, Tmax_list[i])}")

    # 深拷貝可變資源，避免跨批污染
    ROADM_list = copy.deepcopy(roadm_distances)
    Wavelength_list = {k: list(v) for k, v in wavelength_capacity.items()}
    SF_list = list(sf_list_tuples)

    # 修改 BFS：允許 collector 參數
    if not hasattr(bfs, "delay_sf_allocation"):
        raise RuntimeError("你的 bfs_xxx 模組內找不到 delay_sf_allocation")

    bfs.Success, bfs.Fail = 0, 0
    collector = {"delays": [], "powers": []}

    # 靜音跑 BFS
    _stdout = sys.stdout
    sys.stdout = io.StringIO()
    try:
        bfs.delay_sf_allocation(
            Tmax_list, Task_list,
            ROADM_list, Wavelength_list, SF_list, topology_bfs,
            collector=collector  # ← 你已經把 BFS 那支加上 collector 參數了
        )
    finally:
        sys.stdout = _stdout

    total = bfs.Success + bfs.Fail
    success_rate = bfs.Success / total if total else 0.0
    avg_delay = _mean_safe(collector.get("delays", []))
    avg_power = _mean_safe(collector.get("powers", []))
    return {"success_rate": success_rate, "avg_delay": avg_delay, "avg_power": avg_power}
# ====== 靜音版 RL 測試（你之前加過，放這裡以便呼叫）======
def _test_model_performance_silent(env, agent, test_tasks, model_name):
    import io, sys
    _stdout = sys.stdout; buf = io.StringIO()
    try:
        sys.stdout = buf
        results = test_model_performance(env, agent, test_tasks, model_name)
    finally:
        sys.stdout = _stdout
    return results

# ====== 四批任務（easy_10 / medium_10 / hard_10 / mixed_10）一鍵對照 ======
def compare_rl_vs_bfs_on_10task_batches(env, agent, trainer=None, train_rl=False, rl_episodes=50):
    """
    對四個 10 任務批次做 RL vs 真 BFS 的並排比較。
    train_rl=True 時，會先在該批次上訓練 rl_episodes 再評測。
    """
    # 推薦：評測時關掉 Dropout，確保可重現
    if hasattr(agent, "policy_net"): agent.policy_net.eval()
    if hasattr(agent, "value_net"): agent.value_net.eval()

    task_gen = TaskGenerator(env.topology)
    batches = create_10_task_batches()  # {'easy_10': [...], 'medium_10': [...], ...}

    rows = []
    for name, spec in batches.items():
        tasks = task_gen.generate_task_batch(spec)

        # 可選：先訓練 RL，再評測
        if train_rl and trainer is not None:
            for _ in range(rl_episodes):
                trainer.train_single_episode(tasks)

        # 1) 測 RL
        rl = _test_model_performance_silent(env, agent, tasks, f"RL-{name}")['summary']
        # 2) 測 真 BFS（實跑）
        bfs = run_real_bfs_on_tasks(tasks)

        rows.append({
            "batch": name,
            "rl_sr": rl['success_rate'], "rl_delay": rl['avg_delay'], "rl_power": rl['avg_power'],
            "bfs_sr": bfs['success_rate'], "bfs_delay": bfs['avg_delay'], "bfs_power": bfs['avg_power'],
        })

    # 印整齊的對照表
    print("\n=== RL vs 真 BFS：四個 10 任務批次並排比較 ===")
    print("批次\t\tRL成功率\tBFS成功率\tRL功耗\tBFS功耗\tRL延遲\tBFS延遲")
    for r in rows:
        print(f"{r['batch']:<8s}\t{r['rl_sr']:.2%}\t{r['bfs_sr']:.2%}\t{r['rl_power']:.1f}\t{r['bfs_power']:.1f}\t{r['rl_delay']:.4f}\t{r['bfs_delay']:.4f}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 統一解包 env/agent/trainer（無論 main 回傳 3 筆或 4 筆）
        env = agent = trainer = evaluator = None
        if isinstance(result, tuple):
            if len(result) >= 4:
                env, agent, trainer, evaluator = result[:4]
            elif len(result) >= 3:
                env, agent, trainer = result[:3]

        # 成功訊息（保留你原本的印法）
        if env is not None and agent is not None and trainer is not None:
            if evaluator is not None and hasattr(evaluator, "evaluation_results") and evaluator.evaluation_results:
                final_score = evaluator.evaluation_results.get('total_score', None)
                grade = evaluator.evaluation_results.get('grade', None)
                is_better = evaluator.evaluation_results.get('is_better_than_bfs', None)

                print("\n" + "🎉" * 20)
                print("🎉 === 完整系統運行成功! === 🎉")
                print("🎉" * 20)

                if final_score is not None and grade is not None and is_better is not None:
                    print(f"\n🏆 === 最終結果 === 🏆")
            ...
            # ===== 這裡插入 RL vs 真 BFS 的四批次比較 =====
            print("\n" + "="*60)
            print("🧪 RL vs 真 BFS（四個 10 任務批次）")
            print("="*60)
            try:
                # 不追加微調直接比較（想先微調可改 train_rl=True, rl_episodes=50）
                compare_rl_vs_bfs_on_10task_batches(env, agent, trainer=trainer, train_rl=False)
            except NameError as ne:
                print("⚠️ 無法呼叫 compare_rl_vs_bfs_on_10task_batches：請確認該函式、"
                      "_test_model_performance_silent、run_real_bfs_on_tasks 已在檔案上方定義。")
                print(f"   詳細: {ne}")
            except ImportError as ie:
                print("⚠️ 匯入 BFS 實作失敗：請確認你的 BFS 檔名/匯入別名正確，"
                      "且 delay_sf_allocation 已加入 collector 參數並修正縮排。")
                print(f"   詳細: {ie}")
            except Exception as ex:
                print(f"⚠️ RL vs BFS 比較時發生例外: {type(ex).__name__}: {ex}")

        else:
            print("\n⚠️ 系統運行完成，但返回值格式異常（無法取得 env/agent/trainer）")

    except Exception as e:
        print(f"\n❌ 運行過程中出現錯誤: {e}")
        print(f"錯誤類型: {type(e).__name__}")
        print("\n🔧 調試建議:")
        print("1. 檢查是否缺少必要的導入模組")
        print("2. 確認所有函數定義都正確")
        print('3. 嘗試運行簡化版本: python -c "from your_script import quick_demo_original_3_tasks; quick_demo_original_3_tasks()"')
    ...
    print("💡 === 如何確認模型品質 === 💡")
    print("看這些關鍵指標:")
    print("   🎯 成功率提升 > 20% → 證明RL有效學習")
    print("   📊 穩定性分數 > 80 → 證明模型可靠")
    print("   🏆 vs BFS獲勝率 > 60% → 證明超越原算法")
    print("   📈 總評分 > 75 → 證明模型品質優秀")
# ===== 快速使用範例 =====
